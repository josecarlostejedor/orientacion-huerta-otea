  ...
  const calculateResults = () => {
    ...
    if (googleSheetsUrl) {
      const payload = {
        ...
        partialTimes: results.map((r, i) => `B${i+1}: ${formatTime(r.partialTime || 0)}`).join(', '),
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      raceResult.results.forEach((res, idx) => {
        ...
        const timeStr = formatTime(res.partialTime || 0);
                  ...
                  <tbody className="divide-y divide-stone-100">
                    {raceResult.results.map((res, idx) => (
                      <tr key={idx} className="text-sm">
                        ...
                        <td className="p-4 font-mono text-stone-500">{formatTime(res.partialTime || 0)}</td>
                        <td className="p-4">
                          <span className={cn(
